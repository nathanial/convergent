...
test "TwoPGraph add vertex" := do
  let g := TwoPGraph.apply (TwoPGraph.empty : TwoPGraph Nat) (TwoPGraph.addVertex 1)
  (g.containsVertex 1) ≡ true
  (g.vertexCount) ≡ 1

test "TwoPGraph add multiple vertices" := do
  let g := TwoPGraph.empty
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 1)
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 2)
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 3)
  (g.vertexCount) ≡ 3
  (g.containsVertex 1) ≡ true
...
test "TwoPGraph remove vertex" := do
  let g := TwoPGraph.empty
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 1)
    |> fun g => TwoPGraph.apply g (TwoPGraph.removeVertex 1)
...
test "TwoPGraph add edge" := do
  let g := TwoPGraph.empty
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 1)
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 2)
    |> fun g => TwoPGraph.apply g (TwoPGraph.addEdge 1 2)
...
test "TwoPGraph edge requires both endpoints" := do
  let g := TwoPGraph.empty
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 1)
    |> fun g => TwoPGraph.apply g (TwoPGraph.addEdge 1 2)  -- vertex 2 not added
...
test "TwoPGraph remove edge" := do
  let g := TwoPGraph.empty
    ...
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 2)
    |> fun g => TwoPGraph.apply g (TwoPGraph.addEdge 1 2)
    |> fun g => TwoPGraph.apply g (TwoPGraph.removeEdge 1 2)
...
test "TwoPGraph neighbors" := do
  let g := TwoPGraph.empty
    ...
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 3)
    |> fun g => TwoPGraph.apply g (TwoPGraph.addEdge 1 2)
    |> fun g => TwoPGraph.apply g (TwoPGraph.addEdge 1 3)
...
test "TwoPGraph merge combines vertices and edges" := do
  let g1 := TwoPGraph.empty
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 1)
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 2)
    |> fun g => TwoPGraph.apply g (TwoPGraph.addEdge 1 2)
  let g2 := TwoPGraph.empty
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 2)
    |> fun g => TwoPGraph.apply g (TwoPGraph.addVertex 3)
    |> fun g => TwoPGraph.apply g (TwoPGraph.addEdge 2 3)
